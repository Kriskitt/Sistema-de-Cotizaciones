    ...
    <app-role-shell [user]="data.userById('admin')" [navItems]="nav">
      <div class="detail-top mb-4">
        <div class="d-flex align-items-start gap-4 flex-wrap">
          <a class="btn btn-link text-dark fw-bold fs-4 p-0 mt-2" routerLink="/admin/cotizaciones"><i class="bi bi-arrow-left me-2"></i>Volver</a>
          <div>
        ...
        <div class="d-flex gap-3 flex-wrap">
          ...
          <button class="btn btn-dark"><i class="bi bi-send me-2"></i>Enviar al Cliente</button>
        </div>
      </div>
      ...
      <div class="row g-4 mb-4">
        <div class="col-lg-6">
          <section class="section-card card-ui h-100">
            ...
            <div class="info-list">
              @for (item of clientInfo; track item.label) {
                <div class="item">
        ...
        <div class="col-lg-6">
          <section class="section-card card-ui h-100">
            <h2 class="section-title fs-2 mb-4"><i class="bi bi-person me-2"></i>Vendedor Asignado</h2>
            <div class="info-list">
              @for (item of sellerInfo; track item.label) {
                <div class="item">
                  <i [class]="'bi ' + item.icon"></i>
                  <div>
                    <div class="label">{{ item.label }}</div>
...
export class AdminQuotationDetailComponent {
  ...
  readonly seller = this.data.userById(this.quote.sellerId);
  readonly totals = this.calculator.calculate(this.quote.items, this.quote.discountPercent, this.quote.taxPercent);
  readonly clientInfo = [
    { label: 'Nombre', value: this.client.name, icon: 'bi-person' },
    { label: 'Correo', value: this.client.email, icon: 'bi-envelope' },
    { label: 'Teléfono', value: this.client.phone, icon: 'bi-telephone' },
    { label: 'Empresa', value: this.client.company, icon: 'bi-building' }
  ];
  readonly sellerInfo = [
    { label: 'Nombre', value: this.seller.name, icon: 'bi-person' },
    { label: 'Correo', value: this.seller.email, icon: 'bi-envelope' },
    { label: 'Teléfono', value: this.seller.phone, icon: 'bi-telephone' }
  ];
